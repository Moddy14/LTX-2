...
def output_frame_rate(frame_rate: float, temporal_upscalings: int) -> float:
    """Return DFR playback fps without discarding fractional source rates."""
    if frame_rate <= 0:
        raise ValueError(f"frame_rate must be > 0, got {frame_rate}")
    if temporal_upscalings < 0:
        raise ValueError(f"temporal_upscalings must be >= 0, got {temporal_upscalings}")
    return frame_rate * (2**temporal_upscalings)
