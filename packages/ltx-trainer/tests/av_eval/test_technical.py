...
def test_technical_bundle_is_deterministic_and_covers_the_release_surface() -> None:
    ...
    first = build_technical_evidence_bundle(observations, surface=surface)
    second = build_technical_evidence_bundle(copy.deepcopy(observations), surface=copy.deepcopy(surface))
    ...
    assert build_technical_evidence_bundle(observations, surface=surface) == second
...
def test_technical_bundle_rejects_incomplete_or_false_live_evidence() -> None:
    ...
    with pytest.raises(TechnicalEvidenceError, match="exactly cover"):
        build_technical_evidence_bundle(missing_canary, surface=surface)
    ...
    with pytest.raises(TechnicalEvidenceError, match="exactly 20"):
        build_technical_evidence_bundle(short_cycles, surface=surface)
    ...
    with pytest.raises(TechnicalEvidenceError, match="foreign service"):
        build_technical_evidence_bundle(foreign_action, surface=surface)
    ...
    with pytest.raises(TechnicalEvidenceError, match="mode does not match"):
        build_technical_evidence_bundle(wrong_mode, surface=surface)
...
def test_technical_cli_emits_one_qualification_bundle(tmp_path: Path) -> None:
    ...
    assert json.loads(result.stdout)["schema_version"] == "ltx-av-eval-technical-qualification-bundle.v1"
