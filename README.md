...
* **[IDLoraPipeline](packages/ltx-pipelines/src/ltx_pipelines/id_lora.py)** - TalkVid person and speaker identity transfer from reference image and audio.
* **[FLF2VPipeline](packages/ltx-pipelines/src/ltx_pipelines/flf2v.py)** - First/last-frame interpolation with the official deterministic ancestral schedule.
* **[InOutpaintPipeline](packages/ltx-pipelines/src/ltx_pipelines/inoutpaint.py)** - Two-stage masked inpainting and centered outpainting with Laplacian compositing.
...
* **[DubItPipeline](packages/ltx-pipelines/src/ltx_pipelines/dubit.py)** - Dub-It: rephrasing while matching speaker identity and lip movements (distilled model, single IC-LoRA, two stages).
  Existing `LipDubPipeline` imports and the persisted Studio mode ID `lipdub` remain compatibility aliases.
...
LTX-2 pipelines support automatic prompt enhancement via an `enhance_prompt` parameter. A compatible Gemma 3 encode
root is reused; Gemma 4 encoding uses a separate generative instruct checkpoint selected with
`--prompt-enhancer-gemma-root`. Image-conditioned pipelines also provide their reference image to the enhancer.

The CLI flag is `--enhance-prompt`. Prompt enhancement requires the processor assets embedded in, or stored beside,
the selected enhancer checkpoint.

### LTX Studio

The local production UI covers twelve persisted workflow modes, media uploads and existing DGX paths, model
...
Long-running native denoising loops use cooperative, boundary-bound checkpoints. A yielded job resumes only from a
matching fingerprint, generation and diffusion schedule; stale or missing decisions fail closed instead of silently
