/*
 * Candidate-release code is deliberately never an installer. Executing this
 * file through sudo/root fails before a candidate module is imported. The
 * separately installed root-owned bootstrap is the sole privileged authority.
 */
...
try {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    instruction: "An administrator must invoke the fixed external bootstrap; this candidate never runs sudo or privileged install code.",
  })}\n`);
} finally {
  authority?.close();
}
