#!/usr/bin/env python3
"""Restore the exact LTX frame timeline and original audio after MuseTalk."""
...
def restore_timeline(
    ...
    if driving_audio is None and not source_timeline.has_audio:
        raise RuntimeError("The LTX base has no original audio for MuseTalk.")
    if (source_timeline.width, source_timeline.height) != (
        refined_timeline.width,
        refined_timeline.height,
    ):
        raise RuntimeError(
            "MuseTalk changed the resolution: "
    ...
    if driving_audio is None:
        ...
        if not audio_stream.get("streams"):
            raise RuntimeError(f"MuseTalk driving audio has no audio stream: {resolved_audio}")
    ...
    if (
        ...
        raise RuntimeError("MuseTalk output does not match the exact LTX timeline.")
    return restored
