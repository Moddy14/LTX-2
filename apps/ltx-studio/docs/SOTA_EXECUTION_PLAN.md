...
| Python HTTP-/Security-Stack | Die isolierte R1-Runtime pinnt jetzt Requests 2.34.2, Setuptools 84.0.0 und Transformers 5.14.1; aktuelle Einzelprüfungen melden für 76 Python- und 374 Node-Komponenten jeweils null Findings/adverse Status. Der M2-Beleg vom 15.08. bindet jedoch alte Lock-, Pyproject-, Verifier-, Server- und Source-Digests und ist für den finalen Release deterministisch `stale-security-audit`. Das neue fail-closed Gate akzeptiert ausschließlich einen höchstens 24 Stunden alten, extern gespeicherten und durch einen von Authorizer/Finalizer getrennten `qualification-attestor` signierten `ltx-studio-security-audit.v2`. V2 bindet den finalen Release-Digest, beide Locks, Runtime-Pyproject/-Verifier, vorhandene und gehashte Request-/Response-/Normalisierungsartefakte sowie die vollständige release-seitig abgeleitete Local-/Direct-Wheel-/CUDA-Sollmenge. Finalizer und Promotion validieren Dokument, Signatur, Rohartefakte und Binding erneut. Der Runtime-Install-Content-Seal ist implementiert: Die statische Policy bleibt zirkelfrei im Manifest, der anschließend erzeugte dynamische Seal bindet Release-Digest, Locks, Verifier und den vollständig kanonisch gehashten `.venv`-Baum; Install, Startup und Audit prüfen ihn fail-closed einschließlich Root-Ownership/Read-only-Gate | Mechanismus und Negativtests grün; neuer Release bleibt auf Security-HOLD, bis nach dem deterministischen Doppelbuild ein neuer Seal und ein frischer externer V2-Audit exakt gegen den finalen Digest erzeugt und signiert wurden | P0 |
...
Shared-Environment. Davon getrennt war der neue Security-Auditbefund für die
damals gepinnten Paketversionen. Die Pakete sind aktualisiert; der historische
M2-Beleg bleibt wegen Digest- und Cutoff-Drift ausdrücklich ungültig und darf
nicht als Releasebeleg wiederverwendet werden. Vor den
...
schwankender Hostlast misst `startup:measure-paired` Basis und Kandidat in 40
alternierenden Runden mit je neuem Browserkontext, statt zeitliche Lastdrift als
Produktunterschied auszugeben. Doppelbuild und 40+40
...
Scheduler-Actions, exakt 20 Zyklen mit frühen/mittleren/späten Grenzen und
exakt 50 Soak-Jobs mit Nulltoleranz für Verlust, Orphans, Duplikate,
ungebundene Outputs, fremde Serviceaktionen und Recovery-SLO-Verstöße belegen.
Die echten GPU-/Restart-/Fault-Injection-Läufe sind noch nicht erfolgt.
...
Rate günstiger ansetzen. Der aktuelle v2-Bindingreport umfasst deterministisch
301 Planungshypothesen und ist Pflichtevidenz in D0 sowie ein eigenes
detailliertes F0-Dokument. Ein v1/frozen90-Report bleibt bei 193 Hypothesen und
wird von den aktuellen Konsumenten fail-closed abgewiesen. Der offizielle
...
3. Den vorbereiteten `vbench-gates.v2`-Katalog einfrieren: offizieller Commit und Config,
...
4. Simulations-/Präzisionsanalyse auf der unabhängigen Einheit
   ...
   Der ausführbare v2-D0a-Report zählt die 144 claim-spezifischen VBench-Gates
   ...
   damit 301 Hypothesen (`alpha = 0,05 / 301`); die sechs Dimensionsprofile
   müssen dafür jeweils die ungünstigste registrierte Claim-Annahme verwenden.
   Der unveränderte v1/frozen90-Katalog bleibt dagegen historisch 90/193 und
   darf weder als 144 ausgegeben noch für die erweiterte Surface qualifiziert werden.
