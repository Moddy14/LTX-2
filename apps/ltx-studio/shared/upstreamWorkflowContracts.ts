...
// Union-Control modes, while the Lightricks example workflows (CFG++ samplers) bind
// the extended IC-LoRA, LipDub, and T2A paths. Never mix parameters across sources.
...
const UNION_CONTROL_TEMPLATE = template(
  "official-template:ic-lora-union-control",
  "video_ltx2_3_ic_lora.json",
...
export function upstreamWorkflowContractsForRequest(
  ...
  if (request.mode === "ic-lora") {
    if (request.icLora.profile === "union-control") {
      return [structuredClone(UNION_CONTROL_TEMPLATE)];
    }
